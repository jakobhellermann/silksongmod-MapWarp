#!/usr/bin/env bash
# Regenerate mapwarp_respawns.json: every safe respawn point (HazardRespawnMarker + TransitionPoint
# respawn) of every room, normalized to [0,1] within its scene — the format RespawnPoints embeds.
#
...
GAME="${1:-silksong}"
RABEX="${RABEX:-rabex}"
OUT="$(cd "$(dirname "$0")/.." && pwd)/mapwarp_respawns.json"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
